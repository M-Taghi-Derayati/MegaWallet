...
class GetAssetChartDataUseCase @Inject constructor(
    ...
    suspend operator fun invoke(baseSymbol: String, days: String): ResultResponse<List<Pair<Long, Double>>> {
        return marketDataRepository.getHistoricalPrices(baseSymbol, days)
    }
}
...
class LoadAssetChartPointsUseCase @Inject constructor(
    ...
    suspend operator fun invoke(baseSymbol: String, days: String): List<Pair<Long, Double>> {
