 ...
 *
 * ### سیاستِ نمایش در برابرِ ایندکس (گردوغبارِ ایردراپ/کلاهبرداری)
 *
 * ایندکسرِ سرور دیگر `eth_getLogs` را به قراردادهای کاتالوگ محدود نمی‌کند؛ روی آدرس‌های تحتِ نظر
 * تطبیق می‌دهد و توکن را از خودِ log می‌خواند. یعنی **هر** توکنی که به کیف پول برسد ایندکس می‌شود —
 * از جمله ایردراپ‌های تبلیغاتی و کلاهبرداری.
 *
 * قاعده: **همه ایندکس، فقط انتخاب‌شده‌ها نمایش.** و این کلاس تنها نقطهٔ اعمالِ آن است — چیزی به
 * فهرستِ کیف پول می‌رسد که یا در باندلِ کیوریت‌شده باشد یا خودِ کاربر صریحاً اضافه‌اش کرده باشد.
 * توکنِ ناخواسته در پاسخِ کشفِ توکن و در تاریخچه دیده می‌شود (که درست است — کاربر باید بتواند
 * ببیند چه چیزی رسیده)، ولی تا وقتی در [IUserTokenRepository] اضافه نشده هرگز در لیستِ دارایی‌ها،
 * در مجموعِ سبد، یا در انتخابگرِ ارسال ظاهر نمی‌شود. هیچ گاردِ جداگانه‌ای لازم نیست: فیلترنکردن
 * این‌جا خودش یعنی نمایش.
 */
@Singleton
